...
import { IoHardwareChipOutline, IoLocationOutline } from 'react-icons/io5';
import { lorawanAPI } from '../../services/lorawanAPI';

const AssignGeofenceModal = ({ geofence, onClose, onSuccess }) => {
  ...
  const fetchDevices = async () => {
    try {
      ...
      // Fetch already assigned devices
  ...
  const handleAssignDevice = async () => {
    ...
    setLoading(true);
    try {
      await lorawanAPI.assignGeofenceToDevice({
  ...
  const handleRemoveDevice = async (assignmentId, deviceName) => {
    if (window.confirm(`Remove "${deviceName}" from this geofence?`)) {
      try {
        await lorawanAPI.removeDeviceFromGeofence(assignmentId);
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div className="relative inline-block w-full max-w-4xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-xl font-bold text-secondary-700">Assign Devices to Geofence</h3>
              <p className="text-sm text-gray-500">{geofence.geofence_name}</p>
            </div>
          </div>

          {/* Assign New Device */}
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-6">
            <h4 className="text-sm font-semibold text-purple-900 mb-3">Assign New Device</h4>
            <div className="flex gap-3">
              <select
                value={selectedDeviceId}
                onChange={(e) => setSelectedDeviceId(e.target.value)}
                disabled={loadingDevices}
                className="flex-1 px-4 py-2.5 border border-purple-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition text-sm bg-white disabled:bg-gray-100"
              >
              ...
              <button
                ...
                className="px-6 py-2.5 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition font-medium flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <FiPlus />
                Assign
              </button>
            ...
            {availableDevices.length === 0 && !loadingDevices && (
              <p className="text-xs text-purple-700 mt-2">
                All devices are already assigned to this geofence
              </p>
            )}
          </div>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 mb-6 border border-blue-200">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-semibold text-gray-900">{geofence.geofence_name}</p>
                <p className="text-sm text-gray-600">
                  {geofence.geofence_type} • {geofence.organization_name}
                </p>
              </div>
              <div className="text-right">
                ...
                <div className="flex gap-2 mt-1">
                  {geofence.alert_on_enter && (
                    <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded text-xs">Enter</span>
                  )}
                  {geofence.alert_on_exit && (
                    <span className="px-2 py-0.5 bg-orange-100 text-orange-700 rounded text-xs">Exit</span>
                  )}
                </div>
              </div>
            </div>
            ...
            {loadingDevices ? (
              ...
              <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <p className="text-gray-400 text-sm mt-1">
                  Select a device above to assign it to this geofence
                </p>
              </div>
            ) : (
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {assignedDevices.map((device) => (
                  <div
                    ...
                    className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg hover:border-purple-300 transition"
                  >
                    <div className="flex items-center gap-3">
