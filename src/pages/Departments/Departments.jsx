...
// import DepartmentHierarchyModal from '../../components/Department/DepartmentHierarchyModal';
...
//   useEffect(() => {
//     fetchDepartments();
//   }, [pagination.page, filterActive, filterOrganization, filterSite]);
...
//       if (filterSite !== 'all') {
//         const siteId = parseInt(filterSite, 10);
//         if (!isNaN(siteId)) {
...
//         setPagination(prev => ({
//           ...prev,
//           totalItems: Array.isArray(departmentsData) ? departmentsData.length : 0,
//           totalPages: 1,
//         }));
//       }
//     } catch (err) {
...
//       } else {
//         alert(err.message || 'Failed to fetch department details');
//       }
//     }
//   };
...
//     if (filterOrganization === 'all') {
//       alert('Please select an organization to view hierarchy');
//       return;
//     }
...
//       dept.organization_name?.toLowerCase().includes(search) ||
//       dept.site_name?.toLowerCase().includes(search) ||
//       dept.head_employee_name?.toLowerCase().includes(search)
//     );
//   });
...
//             <div>
//               <h1 className="text-2xl font-bold text-secondary-700">Departments</h1>
//               <p className="text-sm text-gray-500 mt-1">Manage departments across your organizations</p>
//             </div>
...
//                           {searchTerm ? 'Try adjusting your search' : 'Click "Add Department" to create your first department'}
//                         </p>
//                       </div>
//                     </td>
//                   </tr>
...
//                             <div className="text-xs text-gray-500">Code: {dept.department_code}</div>
//                           </div>
//                         </div>
//                       </td>
//                       <td className="px-6 py-4">
//                         <div className="text-sm text-gray-900">{dept.organization_name || 'N/A'}</div>
//                         <div className="text-xs text-gray-500">{dept.site_name || 'N/A'}</div>
//                       </td>
//                       <td className="px-6 py-4 whitespace-nowrap">
...
//                             dept.is_active ? 'bg-green-600' : 'bg-red-600'
//                           }`}></span>
...
//                           <button
//                             onClick={() => handleView(dept.department_id)}
...
const Departments = () => {
  ...
  }, [pagination.page, filterActive, filterOrganization, filterSite, orgId]);
  ...
  const handleViewHierarchy = () => {
    if (filterOrganization === 'all' || !orgId || isNaN(orgId)) {
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-secondary-700">Departments</h1>
              <p className="text-sm text-gray-500 mt-1">Manage departments for your organization</p>
            </div>
            <div className="flex gap-3">
              <button
                onClick={handleViewHierarchy}
                disabled={!orgId || isNaN(orgId)}
                className="px-4 py-2 bg-accent-blue text-white rounded-lg hover:bg-accent-teal transition font-medium flex items-center gap-2 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                </svg>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-4 border border-gray-200">
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <select
              ...
              disabled={filterOrganization === 'all' || organizations.length === 0}
              className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition bg-white text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <option value="all">All Sites</option>
              {sites.map(site => (
