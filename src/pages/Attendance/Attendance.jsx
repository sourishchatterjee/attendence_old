  ...
  const handleDelete = async (attendanceId, employeeName) => {
    if (window.confirm(`Are you sure you want to delete attendance record for ${employeeName}?`)) {
      try {
        ...
        console.error('Delete error:', err);
        showNotification(err.message || 'Failed to delete attendance record', 'error');
      }
    }
  };
                      ...
                      filteredAttendance.map((record) => (
                        <tr key={record.attendance_id} className="hover:bg-gray-50 transition">
                          ...
                          <td className="px-4 py-3">
                            <div className="text-sm text-gray-900">{formatTime(record.checkin_time)}</div>
                            {record.checkin_location && (
                              <div className="text-xs text-gray-500 truncate max-w-[120px]" title={record.checkin_location}>
                                {record.checkin_location}
                              </div>
                            )}
                          </td>
                          <td className="px-4 py-3">
                            <div className="text-sm text-gray-900">{formatTime(record.checkout_time)}</div>
                            {record.checkout_location && (
                              <div className="text-xs text-gray-500 truncate max-w-[120px]" title={record.checkout_location}>
                                {record.checkout_location}
                              </div>
                            )}
                          </td>
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {filteredTodayList.length === 0 ? (
                          ...
                          filteredTodayList.map((record, index) => (
                            <tr key={`${record.employee_id}-${index}`} className="hover:bg-gray-50 transition">
                              <td className="px-4 py-3">
                                <div>
                                  <p className="text-sm font-medium text-gray-900">{record.employee_name}</p>
                                  <p className="text-xs text-gray-500">{record.employee_code}</p>
